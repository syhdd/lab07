...
def generate_graph(words):
    ...
    def edit_distance_one(word):
        ...
        for i in range(len(word)):
            left, c, right = word[0:i], word[i], word[i + 1:]
            for replaceletter in lowercase:
                if(replaceletter != c):
                    allCombos.append(list())
                    allCombos[-1].append(replaceletter)
                    for letter in left:
                        allCombos[-1].append(letter)
                    for letter in right:
                        allCombos[-1].append(letter)
        
        for combo in allCombos:
            for result in itertools.permutations(combo,5):
                yield "".join(result)
